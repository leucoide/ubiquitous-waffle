...
int handle_address(node n, artnet_packet p) {
    ...
    for (i = 0; i < ARTNET_MAX_PORTS; i++) {
        if (p->data.addr.swin[i] == PROGRAM_NO_CHANGE) {
            ...
            // reset to defaults
            n->ports.in[i].port_addr = _make_addr(n->state.subnet,
                                                  n->ports.in[i].port_default_addr);
    ...
    for (i = 0; i < ARTNET_MAX_PORTS; i++) {
        if (p->data.addr.swout[i] == PROGRAM_NO_CHANGE) {
            ...
            // reset to defaults
            n->ports.out[i].port_addr = _make_addr(n->state.subnet,
                                                   n->ports.out[i].port_default_addr);
    ...
    switch (p->data.addr.command) {
        ...
        case ARTNET_PC_RESET:
            n->ports.out[0].port_status =
                    n->ports.out[0].port_status & ~PORT_STATUS_DMX_SIP & ~PORT_STATUS_DMX_TEST &
                    ~PORT_STATUS_DMX_TEXT;
...
void handle_rdm(node n, artnet_packet p) {
    ...
    if (n->callbacks.rdm_c.fh != NULL)
        n->callbacks.rdm_c.fh(n, p->data.rdm.address, p->data.rdm.data, ARTNET_MAX_RDM_DATA,
                              n->callbacks.rdm_c.data);
